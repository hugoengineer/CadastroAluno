...
public class CriarTabelaTest {
    ...
    void setUp() throws SQLException {
        connection = DriverManager.getConnection("jdbc:h2:mem:testdb_aluno;DB_CLOSE_DELAY=-1");
        ...
        if (AlunoDAOimpl.alunos != null) {
            AlunoDAOimpl.alunos.clear();
        }
        AlunoDAOimpl.alunoCopia = null;
    }
    ...
    void testInserirAluno() {
        ...
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM turma_aluno WHERE matricula_aluno = 10001")) {
            assertTrue(rs.next());
            assertEquals(1, rs.getInt(1), "Aluno deve ter sido vinculado à turma.");
        } catch (SQLException e) {
            fail("Erro ao verificar turma_aluno: " + e.getMessage());
        }
    }
    ...
    void testDeletarAluno() {
        ...
        alunoDAO.inserirAluno(aluno); // Isso também insere em turma_aluno
